...
export function update(nome6, texto6, id, callback) {
    connection.query("UPDATE feedback6 SET nome6 = ?, texto6 = ?, ativo_feedback = 1 WHERE idfeedback = ?", [nome6, texto6, id], callback);
}
export function deleteF6(id, callback) {
    connection.query('DELETE from feedback6 where idfeedback = ?', [id], callback);
}