...
export function update(nome8, texto8, id, callback) {
    connection.query("UPDATE feedback8 SET nome8 = ?, texto8 = ?, ativo_feedback = 1 WHERE idfeedback = ?", [nome8, texto8, id], callback);
}
export function deleteF8(id, callback) {
    connection.query('DELETE from feedback8 where idfeedback = ?', [id], callback);
}