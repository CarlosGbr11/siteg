...
export function update(nome2, texto2, id, callback) {
    connection.query("UPDATE feedback2 SET nome2 = ?, texto2 = ?, ativo_feedback = 1 WHERE idfeedback = ?", [nome2, texto2, id], callback);
}
export function deleteF2(id, callback) {
    connection.query('DELETE from feedback2 where idfeedback = ?', [id], callback);
}