...
export function update(nome5, texto5, id, callback) {
    connection.query("UPDATE feedback55 SET nome5 = ?, texto5 = ? WHERE idfeedback5 = ?", [nome5, texto5, id], callback);
}
export function deleteF5(id, callback) {
    connection.query('DELETE from feedback5 where idfeedback5 = ?', [id], callback);
}