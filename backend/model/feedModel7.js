...
export function update(nome7, texto7, id, callback) {
    connection.query("UPDATE feedback7 SET nome7 = ?, texto7 = ? WHERE idfeedback7 = ?", [nome7, texto7, id], callback);
}
export function deleteF7(id, callback) {
    connection.query('DELETE from feedback7 where idfeedback7 = ?', [id], callback);
}