...
export function update(nome9, texto9, id, callback) {
    connection.query("UPDATE feedback9 SET nome9 = ?, texto9 = ?, ativo_feedback = 1 WHERE idfeedback = ?", [nome9, texto9, id], callback);
}
export function deleteF9(id, callback) {
    connection.query('DELETE from feedback9 where idfeedback = ?', [id], callback);
}